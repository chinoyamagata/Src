...
/* センサデータ位置 */
#define	imRCV_MEAS_H_POS		3								/* 1CHセンサ測定値 */
...
//static uint8_t SubModbus_RcvVol( void );										/* 電圧 */
//static uint8_t SubModbus_RcvPulse( void );										/* パルス */
...
const ST_SensType_t cSensorType[ 6U ] =
{
	...
	{ ecSensType_Pt,		&SubModbus_RcvSens },				/* 1：測温抵抗体 */
	{ ecSensType_V,			&SubModbus_RcvSens },				/* 2：電圧 */
	{ ecSensType_Pulse,		&SubModbus_RcvSens },				/* 3：パルス */
	{ ecSensType_KTc,		&SubModbus_RcvSens },				/* 4：K熱電対 */
	{ ecSensType_TTc,		&SubModbus_RcvSens },				/* 5：T熱電対 */
};
...
static void SubModbus_RcvAnalys( void )
{
	...
	if( wkChkRes == imOK )														/* 読み出し成功時 */
	{
		...
		if( gvInFlash.mProcess.mModelCode == 0xFFU )
		{
			...
			{
			#if 1
				gvInFlash.mProcess.mModelCode = ecSensType_V;		 /* 電圧表示テスト用 */
				gvDispMode.mNowDisp = ecDispMode_Up1CHDw2CH_V;
			#else
				gvInFlash.mProcess.mModelCode = ecSensType_Pulse; 	 /* パルス表示テスト用 */
				gvDispMode.mNowDisp = ecDispMode_Up1CHDw2CH_P;
			#endif
			}
...
static uint8_t SubModbus_RcvSens( void )
{
	...
	{
//		gvMeasPrm.mMeasVal[ 0U ] = imSndTmpOver;							/* オーバーフロー値を設定 */
	...
	{
//		gvMeasPrm.mMeasVal[ 0U ] = imSndTmpUnder;							/* アンダーフロー値を設定 */
	...
	{
//		gvMeasPrm.mMeasVal[ 0U ] = imSndTmpBurn;							/* バーンアウト値を設定 */
	...
	{
//		gvMeasPrm.mMeasVal[ 0U ] = imSndSnsErr;								/* ハードエラー値を設定 */
		gvMeasPrm.mMeasVal[ 0U ] = SubModbus_ErrValChg( imSndSnsErr );
	...
	{
//		gvMeasPrm.mMeasVal[ 1U ] = imSndTmpInit;		/* CH2を初期化 */
		gvMeasPrm.mMeasVal[ 1U ] = SubModbus_ErrValChg( imSndTmpInit );
		gvMeasPrm.mMeasError[ 1U ] = imNonError;		/* CH2を初期化 */
	}
...
static sint16_t SubModbus_Rcv1ChMeasParse( sint16_t arVal )
{
	...
	{
//		wkDataS16 = imSndTmpOver;										/* オーバーフロー値を設定 */
		wkDataS16 = SubModbus_ErrValChg( imSndTmpOver );
		gvMeasPrm.mMeasError[ 0U ] |= imTmpOver;						/* オーバーフローエラー情報を設定 */
	}
	else if( wkDataS16 < wkDwVal )
	{
//		wkDataS16 = imSndTmpUnder;										/* アンダーフロー値を設定 */
