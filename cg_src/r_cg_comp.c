...
void ApiR_COMP0_Start( void )
{
	PMC4 |= 0x10U;		/* コンパレータポート：コンパレータ */
	PM4 |= 0x10U;
	R_COMP0_Start();
}

void ApiR_COMP0_Stop( void )
{
	R_COMP0_Stop();
	PMC4 &= 0xEFU;		/* コンパレータポート：出力Lo */
	PM4 &= 0xEFU;
	P4 &= 0xEFU;
	PU4 &= 0xEFU;
	POM4 &= 0xEFU;
...
void ApiR_COMP0_End( void )
{
	CMPEN = 0U;		/* stop comparator clock */
}
#pragma section
/* End user code. Do not edit comment generated here */
